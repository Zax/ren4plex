...
var ren4plex = {
    ...
    config : {
        ...
        parseYear : true,
        parseEpisode: true,
        preReplace : [ { search: /H.265/gi, replace: '' } ],
        ignoreWords : [
            '','480p','720p','1080p',
            'ITA','ENG','Subs','Sub','iTALiAN','jap','ENGLiSH',
            'hdtv','BluRay','DLMux','BDMux','BRMux','DVDRip','BrRip','BdRip','x264','x265','h264','h265','xvid','MP4','WEBRIP','PROPER','WEB','DL','DMux','Mux',
            ...
            'FoV','IGM','SATOSHi','TLA','RiVER','oRo','by','IperB','rarbg','xXTenGXx','ASAP','TRL','NAHOM','BLUWORLD'
        ]
    },
    ...
    preReplace: function (text){
        ...
        for (var i = 0; i < this.config.preReplace.length; i++) {
            res = text.replace(this.config.preReplace[i].search,this.config.preReplace[i].replace)
        }
        return res;
    },
